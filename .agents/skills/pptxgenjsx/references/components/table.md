...
```tsx
<Table x={0.8} y={1.6} w={11.733} h={4.5}
  ...
  {[
    ...
    ["Product", "Q1", "Q2", "Q3", "Q4"].map((h, i) => (
      <TableCell key={i} bold fontSize={12} color="FFFFFF"
        fill={{ color: "1E293B" }} align="center" valign="middle">
        {h}
      </TableCell>
    )),
    ...
    ...["Widget A", "Widget B", "Gadget X", "Gadget Y"].map((name, ri) =>
      ["$4,500", "$5,200", "$4,800", "$5,600"].map((v, ci) => {
        ...
        return (
          <TableCell key={`${ri}-${ci}`} fontSize={11} color="334155"
    ...
    ...["Total", "$22,500", "$25,800", "$24,100", "$28,400"].map((v, i) => (
      <TableCell key={`total-${i}`} bold fontSize={12} color="1E293B"
        fill={{ color: "EDE9FE" }}
