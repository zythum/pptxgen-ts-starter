...
> 💡 **Tip: add `margin` to text on a colored background**
>
> When a `<Text>` sits on a colored background (e.g. `<Rect>` / `<RoundRect>`),
> **always give the `<Text>` a `margin`** — otherwise the text hugs the shape
> edge and looks cramped.
>
> ```tsx
